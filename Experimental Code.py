...
from time import ticks_us, ticks_diff, sleep_ms, time
...
log_file.write("Time (s), Roll (deg), Pitch (deg), Yaw (deg), Servo1 (deg), Servo2 (deg)\n")
log_file.flush()
boot_time = ticks_us()
...
while True:
    ...
    elapsed = ticks_diff(now, boot_time) / 1_000_000.0  # in seconds

    # Update yaw after 10s
    ...
    # === Logging
    log_file.write(f"{elapsed:.2f}, {roll:.2f}, {pitch:.2f}, {yaw:.2f}, {servo1_angle:.2f}, {servo2_angle:.2f}\n")

    if time() - last_flush_time >= 10:
        log_file.flush()
        last_flush_time = time()
        print(f"[LOG] ⏱ {elapsed:.2f}s | Roll: {roll:.1f}° | Yaw: {yaw:.1f}°")
