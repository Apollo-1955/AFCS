...
log_file.write("Timestamp (s),   Altitude (m),   Roll (deg),   Pitch (deg),   Velocity Z (deg/s),   Alt Derivative (m/s),   Servo Angle (deg),   Event\n\n")
log_file.flush()
...
while True:
    try:
        ...
        # === Logging ===
        log_file.write(
            f"{timestamp:.2f},           {altitude:.2f},          {roll:.2f},         {pitch_angle:.2f},           {velocity_z:.2f},                {dz:.2f},             {servo_deg:.2f},         {event}\n"
        )
        ...
        print(f"⏱ {timestamp:.2f}s | Alt: {altitude:.2f} m | dZ/dt: {dz:.2f} m/s | Pitch: {pitch_angle:.2f}°", end="\r")
