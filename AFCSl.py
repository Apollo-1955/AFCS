...
# === GPS UART Setup ===
gps = UART(0, baudrate=9600, tx=None, rx=13)
gps_buffer = b''
gps_connected = True
gps_lat = gps_lon = gps_fix = gps_sats = gps_alt = gps_speed = 'N/A'
...
def parse_gprmc(sentence):
    ...
    try:
        ...
        return speed_knots * 0.514444  # Convert to m/s
    except:
        return None
...
log_file.write("Time (s), Alt (m), Roll (°), Pitch (°), GyroZ (°/s), AltRate (m/s), Event, Lat, Lon, Fix, Sats, GPS Alt, GPS Speed (m/s)\n")
log_file.flush()
last_flush_time = time()

# === Flight Detection ===
liftoff_detected = apogee_detected = landed_detected = False
stable_time = 0
prev_altitude = 0
...
while True:
    try:
        ...
        if not liftoff_detected and vertical_accel > 0.5:
            ...
            event = "🚀 Liftoff"
        if liftoff_detected and not apogee_detected and dz < 0:
                ...
                while b'\n' in gps_buffer:
                    ...
                    try:
                        ...
                        if line_str.startswith('$GPGGA'):
                            ...
                            if gpgga_data:
                                gps_lat, gps_lon, gps_fix, gps_sats, gps_alt = gpgga_data
                        elif line_str.startswith('$GPRMC'):
                            gps_speed = parse_gprmc(line_str)
                    except:
                        continue
        except Exception as gps_err:
            gps_lat = gps_lon = gps_fix = gps_sats = gps_alt = gps_speed = 'N/A'
            gps_connected = False
            print("GPS read error:", gps_err)

        # === Logging ===
        log_file.write(f"{timestamp:.2f},{altitude:.2f},{roll:.2f},{pitch_angle:.2f},{velocity_z:.2f},{dz:.2f},{event},{gps_lat},{gps_lon},{gps_fix},{gps_sats},{gps_alt},{gps_speed}\n")
        ...
        print(f"⏱ {timestamp:.2f}s | Alt: {altitude:.2f} m | dZ/dt: {dz:.2f} | Pitch: {pitch_angle:.2f}° | GPS: Lat {gps_lat} Lon {gps_lon} Sats {gps_sats}", end="\r")
