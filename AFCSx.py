...
def try_connect_bmp180():
    ...
    try:
        ...
        bmp.sea_level_pressure = 101325  # Will adjust after calibration
...
def calibrate_imu(samples=200):
    ...
    # For accelerometer, remove gravity from Z offset (assuming device stationary flat)
    accel_offsets[2] -= 1.0  # gravity approx 1 g
...
# === BMP180 Calibration ===
# We'll read baseline pressure to adjust sea level pressure for altitude zeroing
...
if baseline_pressure is not None:
    bmp.sea_level_pressure = baseline_pressure  # Zero altitude relative to current pressure
...
while True:
    try:
        # === Sensor Read with calibration offsets applied ===
        ...
        if bmp:
            try:
                ...
                bmp = None  # mark as disconnected
                altitude = None
        else:
            # Retry BMP180 connection every bmp_retry_interval ms
            now = ticks_ms()
        ...
        log_file.write(
            f"{timestamp:.2f},           {altitude_str},          {roll:.2f},         {pitch_angle:.2f},           {velocity_z:.2f},             {servo_deg:.2f},         {event}\n"
        )
